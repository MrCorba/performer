...
CurveSequenceEditPage::CurveSequenceEditPage(PageManager &manager, PageContext &context) : BasePage(manager, context)
{
...
void CurveSequenceEditPage::keyPress(KeyPressEvent &event)
{
    ...
    if (key.isStep() && layer() == Layer::Gate)
    {
...
void CurveSequenceEditPage::contextShow()
{
    showContextMenu(ContextMenu(
        ...
        { contextAction(index); },
        [&](int index)
        { return contextActionEnabled(index); }));
}
